...
import { deleteTenantNote } from "~/actions/tenants/delete-tenant-note";
...
export type DeleteTenantNoteModalProps = NiceModalHocProps & {
  note: TenantNoteDto;
};

export const DeleteTenantNoteModal =
  NiceModal.create<DeleteTenantNoteModalProps>(({ note }) => {
    ...
    const title = "Excluída esta nota?";
    const canSubmit =
    ...
    const onSubmit: SubmitHandler<DeleteTenantSchema> = async (values) => {
      ...
      const result = await deleteTenantNote(values);
      if (!result?.serverError && !result?.validationErrors) {
        toast.success("Nota excluída com sucesso");
        modal.handleClose();
      } else {
        toast.error("Nota não excluída, tente novamente");
      }
    };
