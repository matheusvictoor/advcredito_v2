...
import { DayOfWeek } from "@workspace/database";
        ...
        {hasTimeSlots ? (
          <div className="flex flex-row justify-between">
            ...
            <div className="flex h-9 flex-row items-center">
              ...
              <CopyTimesMenu name={name} sourceDayOfWeek={dayOfWeek} />
            </div>
          </div>
        ) : (
    ...
    return (
      <div className="flex flex-col gap-2">
        <div className="flex items-center gap-2">
          <TimeSlotField value={value} onChange={onChange} />
          <Tooltip delayDuration={0}>
            <TooltipTrigger asChild>
              <Button
                type="button"
...
const TimeSlotField = React.memo(
  ({ value, onChange }: TimeSlotFieldProps): React.JSX.Element => {
    ...
    return (
      <div className="flex items-center gap-2">
        <TimeSelect value={value.start} onChange={handleStartChange} />
        <span>-</span>
        <TimeSelect value={value.end} onChange={handleEndChange} />
      </div>
    );
  },
);
...
const TimeSelect = React.memo(
  ({ value, onChange }: TimeSelectProps): React.JSX.Element => {
    ...
    return (
      <Select value={formattedValue} onValueChange={onChange}>
