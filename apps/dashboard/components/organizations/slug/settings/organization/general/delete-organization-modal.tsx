...
export const DeleteOrganizationModal =
  NiceModal.create<DeleteOrganizationModalProps>(() => {
    ...
    const title = "Deletar organização?";
    const description =
    ...
    const onSubmit: SubmitHandler<DeleteOrganizationSchema> = async () => {
      ...
      if (!result?.serverError && !result?.validationErrors) {
        ...
        toast.error("Não foi possível deletar a organização");
      }
    };
    const renderForm = (
      <form
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input {...field} disabled={methods.formState.isSubmitting} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
          ...
          render={({ field }) => (
            <FormItem className="mx-1 flex flex-row items-center gap-3 space-y-0">
              ...
              <FormLabel className="leading-2 cursor-pointer">
                Não poderei mais acessar a organização e seus dados
              </FormLabel>
            </FormItem>
          )}
        />
