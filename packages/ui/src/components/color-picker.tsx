  ...
  (
    ...
    return (
      ...
      >
        ...
        >
          {showPointer && <Pointer {...pointerProps} {...posProps} />}
        </Interactive>
      </div>
    );
  },
...
function EyeDropper(props: EyeDropperProps): React.JSX.Element {
  ...
  return (
    <Button
      ...
      onClick={handleClick}
    >
      <svg viewBox="0 0 512 512" className="size-4 shrink-0">
        <path
          fill="currentColor"
...
const ChromeInputRGBA = React.forwardRef<HTMLDivElement, ChromeInputRGBAProps>(
  ({ hsva, rProps, gProps, bProps, aProps, onChange, ...other }, ref) => {
    ...
    return (
      <div ref={ref} className="flex flex-row items-center gap-1" {...other}>
        <ChromeInput
          label="R"
          value={rgba.r || 0}
          onBlur={handleBlur}
...
const Chrome = React.forwardRef<HTMLDivElement, ChromeProps>(
  (
    ...
    return (
      <div ref={ref} className={cn("flex w-60 flex-col", className)} {...other}>
        <Saturation
          hsva={hsva}
          onChange={(newColor) => {
